...
def evaluate_model(model, dataloader_test):
    with torch.no_grad():
        ...
        for i_batch, sampled_batch in enumerate(sketch_array_tests):
            ...
            avererage_area.append(mean_rank)
            avererage_area_percentile.append(mean_rank_percentile)

        meanA = [sum(col) / len(col) for col in zip(*avererage_area)]
        meanB = [sum(col) / len(col) for col in zip(*avererage_area_percentile)]
