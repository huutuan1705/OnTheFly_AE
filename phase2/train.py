...
def train_model(model, args):
    ...
    for i_epoch in range(args.epochs):
        ...
        for i, sanpled_batch in enumerate(tqdm(model.Sketch_Array_Train)):
            ...
            sketch_features = model.attn(sanpled_batch.unsqueeze(0)).squeeze(0)
            ...
            if ((i + 1) % 64 == 0) or (i+1 == len(model.Sketch_Array_Train)):
                ...
                utils.clip_grad_norm_(model.attn.parameters(), 40)
                optimizer.step()
                loss_buffer = []
    
        with torch.no_grad():
            top1_eval, top5_eval, top10_eval, meanA, meanB, meanOurA, meanOurB  = model.evaluate_assa()
            model.attn.train()
